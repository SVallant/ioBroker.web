...
function initAuth(server, settings) {
    ...
    server.app.use(session({
        ...
        resave: true,
        cookie: { maxAge: settings.ttl * 1000, httpOnly: false, sameSite: 'None', secure: true }, // default TTL
        store
    }));
...
async function initWebServer(settings) {
        ...
        */
        ...
        if (settings.auth) {
            ...
            server.app.options('*', (req, res) => {
                adapter.log.info("handling OPTIONS request #1");
                //adapter.log.info(JSON.stringify(req.headers));
        ...
        // Enable CORS
        if (settings.socketio || true) {
            adapter.log.info(JSON.stringify(settings.socketio));
            adapter.log.info("socketio enabled => enable cors");
            server.app.use((req, res, next) => {
                res.header('Access-Control-Allow-Origin', req.get("origin") || '*');
                res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS');
                res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Content-Length, X-Requested-With');
                ...
                if ('OPTIONS' === req.method) {
                    adapter.log.info("handling OPTIONS request #2");
                    res.sendStatus(200);
                } else {
                    next();
                }
    ...
    if (ownSocket) {
        ...
        try {
            ...
            adapter.log.debug('require IOSocket: ' + parts.join('/'));
                    ...
                    if (settings.cache && cache[`${id}/${url}`] && !noFileCache) {
                        ...
                        if (id === 'login' && url === 'index.html') {
                            ...
                            adapter.readFile(id, webByVersion[id] && versionPrefix ? url.substring(versionPrefix.length + 1) : url, { user: req.user ? 'system.user.' + req.user : settings.defaultUser, noFileCache: noFileCache }, (err, buffer, mimeType) => {                               
                                ...
                                adapter.log.debug("command readFile: " + id);
